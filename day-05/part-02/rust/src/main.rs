...
fn main() {
  ...
  for t in transitions {
    ...
    }else if t.from.y == t.to.y {
      let (start, end) = if t.from.x < t.to.x {(t.from.x, t.to.x)} else {(t.to.x, t.from.x)};
      for i in start as usize..=end as usize {
        results[i][t.from.y as usize] += 1;
      }
    }else {
      let (left_point, right_point) = if t.from.x < t.to.x {(t.from, t.to)} else {(t.to, t.from)};
      let distance = right_point.x - left_point.x + 1;
      if left_point.y < right_point.y {
        for i in 0..distance as usize {
          results[left_point.x as usize + i][left_point.y as usize + i] += 1;
        }
      } else {
        for i in 0..distance as usize {
          results[left_point.x as usize + i][left_point.y as usize - i] += 1;
